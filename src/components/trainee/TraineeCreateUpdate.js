...
const CreateUpdateTrainee = (props) => {
  ...
  const saveTrainee = ({ firstName, lastName, email, address, mobile, headTrainer, group, feedback, guid }) => {
    ...
    axios.post(`${api.academiaDigitalUrl}${type.putTrainee}guid=${guid}&firstName=${firstName}
    &lastName=${lastName}&email=${email}&address=${address}&mobile=${mobile}&headTrainer=${headTrainer}
    &group=${group}&feedback=${feedback}`)
    .then(() => console.info('Update Success'))
    .catch(error => console.log('error', error));
  }
  const trainee = props.trainee ? props.trainee : newTrainee;
  return <TraineeForm saveTrainee={saveTrainee} trainee={trainee} /> 
}
