...
export default function CustomizedTables({ headers, data, updateList }) {
  ...
  const showModalDelete = () => {
    return (
      <React.Fragment>
        <Modal open={isVisibleDelete} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
          <Box sx={style}>
            <Typography id="modal-modal-title" variant="h6" component="h2">
            ...
            <ViewButton
              ...
              onClick={() => {
                axios
                  .delete(`https://academia-digital-api-ms.azurewebsites.net/api/v1/DeleteById?uuid=${currentTrainee}`)
                  .then(() => {
            ...
            <ViewButton color="#652341" icon={IoCloseCircleSharp} onClick={() => setIsVisibleDelete(false)}>
              Cancel
            </ViewButton>
          </Box>
        </Modal>
            ...
            <TableBody>
              {data.map((element, i) => (
                <StyledTableRow key={i}>
                  ...
                  <StyledTableCell align="left">{element.address}</StyledTableCell>
                  <StyledTableCell align="left">{element.mobile}</StyledTableCell>
                  <StyledTableCell align="left">
                    <ViewButton
                      color="#045bc3"
                      icon={MdPageview}
                      onClick={() => {
                        setCurrentTrainee(element);
                    ...
                    <ViewButton
                      ...
                      icon={FaTrash}
                      onClick={() => {
                        setCurrentTrainee(element.guid);
